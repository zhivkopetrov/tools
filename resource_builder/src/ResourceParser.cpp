...
int32_t ResourceParser::setupResourceTree() {
  ...
  projectAbsBuildFilePath.append(_projectFolder).append("/").
      append(ResourceFileHeader::getResourcesFolderName()).append("/").
      append(ResourceFileHeader::getAutoGeneratedFolderName());
...
int32_t ResourceParser::processAllFiles() {
  ...
  if (SUCCESS != FileSystemUtils::getAllFilesInDirectoryRecursively(
...
int32_t ResourceParser::buildResFileInternalData() {
  ...
  const std::string absResourceFolderPath =
      _currAbsFilePath.substr(0, slashPos + 1);
  ...
  _currDestFile.append(ResourceFileHeader::getAutoGeneratedFolderName())
               .append("/").append(fileNameNoExtension).append("Resources");
...
int32_t ResourceParser::fillPath(const std::string& relativeFilePath,
                                 CombinedData& outData) {
  if (std::string::npos == relativeFilePath.find(EXTERNAL_PATH_PREFIX)) {
    ...
    _fileParser.setCompleteFilePathFromProject(relativeFilePath.substr(
        EXTERNAL_PATH_PREFIX_SIZE,
